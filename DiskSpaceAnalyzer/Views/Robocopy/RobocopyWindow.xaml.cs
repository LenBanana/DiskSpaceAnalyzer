...
public partial class RobocopyWindow : Window
{
    ...
    private void UpdateWizardStep()
    {
        ...
        Line1.Fill =
            new SolidColorBrush(_currentStep > 1 ? Color.FromRgb(0x41, 0x96, 0xF3) : Color.FromRgb(0x55, 0x55, 0x55));
        Line2.Fill =
            new SolidColorBrush(_currentStep > 2 ? Color.FromRgb(0x41, 0x96, 0xF3) : Color.FromRgb(0x55, 0x55, 0x55));
        Line3.Fill =
            new SolidColorBrush(_currentStep > 3 ? Color.FromRgb(0x41, 0x96, 0xF3) : Color.FromRgb(0x55, 0x55, 0x55));
        Line4.Fill =
            new SolidColorBrush(_currentStep > 4 ? Color.FromRgb(0x41, 0x96, 0xF3) : Color.FromRgb(0x55, 0x55, 0x55));
    ...
    private void UpdateStepIndicator(int stepNumber, bool isActive, bool isCompleted)
    {
        ...
        var label = FindName($"Step{stepNumber}Label") as TextBlock;
        var circleChild = circle?.FindName("PART_Content") as TextBlock;

        if (circle == null || label == null) return;

        // Get the TextBlock inside the Border
        var textBlock = FindVisualChild<TextBlock>(circle);

        if (isCompleted)
        {
            // Completed: Green with checkmark
            circle.SetResourceReference(StyleProperty, "StepIndicatorCompleted");
            if (textBlock != null)
            {
                textBlock.Text = "✓";
        ...
        {
            // Active: Blue
            circle.SetResourceReference(StyleProperty, "StepIndicatorActive");
            if (textBlock != null)
            {
        ...
        {
            // Inactive: Gray
            circle.SetResourceReference(StyleProperty, "StepIndicatorInactive");
            if (textBlock != null)
            {
                textBlock.Text = stepNumber.ToString();
