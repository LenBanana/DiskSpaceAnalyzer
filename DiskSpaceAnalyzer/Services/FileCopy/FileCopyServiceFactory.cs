...
public class FileCopyServiceFactory : IFileCopyServiceFactory
{
    ...
    public IFileCopyService CreateService(FileCopyOptions options)
    {
        ...
        if (options.PreferredEngine != CopyEngineType.Auto)
        {
            return CreateServiceByType(options.PreferredEngine);
        }
    ...
    public CopyEngineType[] GetAvailableEngines()
    {
        ...
        {
            available.Add(CopyEngineType.Robocopy);
        }
    ...
    public EngineRecommendation RecommendEngine(FileCopyOptions options)
    {
        ...
        if (options.BackupMode)
        {
            if (robocopyAvailable)
            {
                return new EngineRecommendation
                {
                    ...
                    Reasoning = "Backup mode required - only Robocopy supports copying locked files with SeBackupPrivilege (/B flag).",
                    Confidence = 1.0,
                    Alternatives = new Dictionary<CopyEngineType, string>
                    {
                        { CopyEngineType.Native, "Not supported - cannot copy locked files without Volume Shadow Copy integration." }
                    },
            ...
            else
            {
                return new EngineRecommendation
                {
                    ...
                    Reasoning = "Backup mode requested but Robocopy unavailable. Native engine will be used but locked files cannot be copied.",
                    Confidence = 0.3,
        ...
        if (options.CopySecurity)
        {
            if (robocopyAvailable)
            {
                ...
                {
                    RecommendedEngine = CopyEngineType.Robocopy,
                    Reasoning = "Security information copying requested - Robocopy has comprehensive NTFS ACL and ownership support (/SEC, /COPYALL flags).",
                    Confidence = 0.9,
                    Alternatives = new Dictionary<CopyEngineType, string>
                    {
        ...
        if (sourceIsNetwork || destIsNetwork)
        {
            if (robocopyAvailable)
            {
                ...
                {
                    RecommendedEngine = CopyEngineType.Robocopy,
                    Reasoning = "Network path detected - Robocopy provides superior reliability with automatic retry logic, restartable mode, and decades of network optimization.",
                    Confidence = 0.85,
                    Alternatives = new Dictionary<CopyEngineType, string>
                    {
                        { CopyEngineType.Native, "Will work but lacks specialized network retry logic. May fail on unreliable connections." }
                    },
            ...
            else
            {
                ...
                {
                    RecommendedEngine = CopyEngineType.Native,
                    Reasoning = "Network path detected but Robocopy unavailable. Native engine will be used with basic retry logic.",
                    Confidence = 0.6,
                    Warnings = new List<string>
                    {
        ...
        if (CopyScenarioAnalyzer.WouldBenefitFromNativeSpeed(sourcePath, destPath))
        {
            ...
            if (robocopyAvailable)
            {
                alternatives.Add(CopyEngineType.Robocopy, "More reliable for edge cases but 2-4x slower for local SSD operations.");
            }

            return new EngineRecommendation
            {
                RecommendedEngine = CopyEngineType.Native,
                Reasoning = "Local drive operation - Native C# engine provides 2-4x faster performance with direct async I/O, no process overhead, and real-time byte-level progress tracking.",
                Confidence = 0.85,
        ...
        if (robocopyAvailable)
        {
            ...
            {
                RecommendedEngine = CopyEngineType.Robocopy,
                Reasoning = "Default selection - Robocopy provides proven reliability for general-purpose file copying.",
                Confidence = 0.7,
                Alternatives = new Dictionary<CopyEngineType, string>
                {
                    { CopyEngineType.Native, "Would be faster for local operations but Robocopy chosen for reliability." }
                },
    ...
    private IFileCopyService CreateServiceByType(CopyEngineType engineType)
    {
        switch (engineType)
        {
            case CopyEngineType.Robocopy:
                if (IsRobocopyAvailable())
                {
                    return _robocopyService as IFileCopyService ?? _nativeService;
                }
                else
                {
    ...
    private CopyEngineType SelectOptimalEngine(FileCopyOptions options)
    {
        ...
        if (options.BackupMode && IsRobocopyAvailable())
        {
            return CopyEngineType.Robocopy;
        }

        if (options.CopySecurity && IsRobocopyAvailable())
        {
            return CopyEngineType.Robocopy;
        }
        ...
        if ((sourceIsNetwork || destIsNetwork) && IsRobocopyAvailable())
        {
            return CopyEngineType.Robocopy;
        }
    ...
    private bool IsRobocopyAvailable()
    {
        try
        {
            ...
            var robocopyServiceInterface = _robocopyService as IRobocopyService;
    ...
    private static string GetPathTypeDescription(string path, bool isNetwork)
    {
        ...
        if (driveType.HasValue)
        {
            ...
            {
                System.IO.DriveType.Fixed => "Local",
