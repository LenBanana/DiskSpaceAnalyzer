...
public class FileIntegrityService : IFileIntegrityService
{
    ...
    private readonly List<IntegrityCheckResult> _results = new();
    private readonly object _resultsLock = new();
    private readonly object _statsLock = new();
    
    private readonly OptimizedHashCalculator _hashCalculator = new();
    private readonly ConcurrentBag<string> _queueFailures = new();
    ...
    public void Start(IntegrityCheckMethod method, string sourcePath, string destinationPath, long totalFiles, CancellationToken cancellationToken)
    {
        ...
        for (int i = 0; i < SmallFileConcurrency; i++)
        {
            _workerTasks.Add(Task.Run(() => SmallFileWorkerAsync(_cancellationTokenSource.Token), _cancellationTokenSource.Token));
        }
        
        // Start large file workers (lower concurrency)
        for (int i = 0; i < LargeFileConcurrency; i++)
        {
            _workerTasks.Add(Task.Run(() => LargeFileWorkerAsync(_cancellationTokenSource.Token), _cancellationTokenSource.Token));
        }
        
        // Start retry worker
        _workerTasks.Add(Task.Run(() => RetryWorkerAsync(_cancellationTokenSource.Token), _cancellationTokenSource.Token));
    }
    ...
    public IntegrityProgress GetProgress()
    {
        lock (_statsLock)
        {
            ...
            return new IntegrityProgress
            {
                ...
                StartTime = _verificationTimer.Elapsed == TimeSpan.Zero ? DateTime.Now : DateTime.Now - _verificationTimer.Elapsed,
    ...
    private async Task VerifyFileAsync(FileCopyInfo fileInfo, CancellationToken cancellationToken)
    {
        ...
        try
        {
            ...
            else
            {
                ...
                result.IsValid = _method switch
                {
                    ...
                    IntegrityCheckMethod.MD5 => await VerifyHashAsync(fileInfo, result, HashAlgorithmName.MD5, cancellationToken),
                    IntegrityCheckMethod.SHA256 => await VerifyHashAsync(fileInfo, result, HashAlgorithmName.SHA256, cancellationToken),
                    IntegrityCheckMethod.XXHash64 => await VerifyHashAsync(fileInfo, result, OptimizedHashCalculator.XXHash64, cancellationToken),
                    IntegrityCheckMethod.Blake3 => await VerifyHashAsync(fileInfo, result, OptimizedHashCalculator.Blake3, cancellationToken),
                    _ => false
                };
            }
        }
        ...
        catch (Exception ex) when (fileInfo.AttemptCount < MaxRetryAttempts)
        {
            ...
            System.Diagnostics.Debug.WriteLine($"Verification error (attempt {fileInfo.AttemptCount + 1}/{MaxRetryAttempts}): {ex.GetType().Name} - {ex.Message}");
    ...
    private async Task<bool> VerifyMetadataAsync(FileCopyInfo fileInfo, IntegrityCheckResult result, CancellationToken cancellationToken)
    {
        await Task.Run(() =>
        {
    ...
    private async Task<bool> VerifyHashAsync(FileCopyInfo fileInfo, IntegrityCheckResult result, HashAlgorithmName algorithmName, CancellationToken cancellationToken)
    {
        try
        {
            // Use optimized hash calculator with buffer pooling and memory-mapped I/O
            var sourceHash = await _hashCalculator.CalculateFileHashAsync(fileInfo.SourceFullPath, algorithmName, cancellationToken);
            result.ExpectedHash = sourceHash;
            
            var destHash = await _hashCalculator.CalculateFileHashAsync(fileInfo.DestinationFullPath, algorithmName, cancellationToken);
    ...
    public List<IntegrityCheckResult> GetResults()
    {
        lock (_resultsLock)
        {
            return new List<IntegrityCheckResult>(_results);
        }
    }
