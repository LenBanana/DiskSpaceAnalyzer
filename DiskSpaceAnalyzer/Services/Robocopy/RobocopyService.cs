...
public class RobocopyService : IFileCopyService, IRobocopyService
{
    ...
    public List<IntegrityCheckResult>? GetCurrentVerificationResults()
    {
    ...
    /// </summary>
    private bool ShouldExcludeDirectory(string fullPath, string directoryName, string sourcePath,
        List<string> excludedDirectories)
    {
        ...
        foreach (var excluded in excludedDirectories)
            ...
            else if (excluded.Contains("\\"))
            {
                ...
                var relativePath = GetRelativePath(sourcePath, fullPath);
    ...
    private void OnOutputDataReceived(object sender, DataReceivedEventArgs e)
    {
        ...
        lock (_outputLock)
        {
            ...
            if (_outputLines.Count > 1000) _outputLines.RemoveRange(0, 500);
        }
    }
    ...
    /// </summary>
    Task<(bool IsValid, string? ErrorMessage, List<string>? Warnings)> IFileCopyService.ValidateOptionsAsync(
        FileCopyOptions options)
    {
        var robocopyOptions = FileCopyOptionsMapper.ToRobocopyOptions(options);
        ...
        if (robocopyOptions.MirrorMode)
            warnings = ["⚠️ Mirror mode will delete files at destination that don't exist at source"];
