...
public partial class RobocopyLogParser
{
    ...
    [GeneratedRegex(@"^\s+Dirs\s*:\s*(\d+)\s+(\d+)\s+(\d+)\s+(\d+)\s+(\d+)\s+(\d+)",
        RegexOptions.Compiled | RegexOptions.IgnoreCase)]
    private static partial Regex DirsSummaryRegex();

    [GeneratedRegex(@"^\s+Files\s*:\s*(\d+)\s+(\d+)\s+(\d+)\s+(\d+)\s+(\d+)\s+(\d+)",
        RegexOptions.Compiled | RegexOptions.IgnoreCase)]
    private static partial Regex FilesSummaryRegex();

    [GeneratedRegex(@"^\s+Bytes\s*:\s*(\d+)\s+(\d+)\s+(\d+)\s+(\d+)\s+(\d+)\s+(\d+)",
        RegexOptions.Compiled | RegexOptions.IgnoreCase)]
    ...
    public string? ParseCurrentFile(IEnumerable<string> recentLines)
    {
        ...
        foreach (var line in recentLines)
        {
                ...
                !line.StartsWith("ERROR"))
            {
                ...
                if (parts.Length >= 3) lastFile = parts[^1]; // Last element is the path
            }
    ...
    public (string? filePath, long fileSize) ParseFileInfo(string line)
    {
        ...
        if ((line.Contains("New File") ||
            ...
            try
            {
                ...
                var pathStartIndex = sizeIndex + 3; // size + date + time = path starts here
    ...
    public (long fileCount, long estimatedBytes) EstimateProgress(string logContent, long averageFileSize)
    {
        ...
        if (totalBytes == 0 && fileCount > 0) totalBytes = fileCount * averageFileSize;
    ...
    {
        var success = exitCode < 8; // Exit codes 0-7 are considered success
        var message = RobocopyResult.InterpretExitCode(exitCode);
