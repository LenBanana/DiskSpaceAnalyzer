...
public class RobocopyCommandBuilder
{
    ...
    public string BuildArguments(RobocopyOptions options)
    {
        ...
        {
            args.Add("/E"); // Copy subdirectories including empty ones
        }
        
        // Mirror mode (DANGEROUS - deletes at destination)
        if (options.MirrorMode)
        {
            args.Add("/MIR"); // Mirror mode
        }
        ...
        // Copy flags
        if (!string.IsNullOrEmpty(options.CopyFlags))
        {
            args.Add($"/COPY:{options.CopyFlags}");
        }
        ...
        // Symbolic links
        if (options.CopySymbolicLinks)
        {
            args.Add("/SL");
        }
        
        // Create tree only
        if (options.CreateTreeOnly)
        {
            args.Add("/CREATE");
        }
        ...
        else if (options.MoveFilesAndDirectories)
        {
            args.Add("/MOVE");
        }
        ...
        {
            args.Add($"/MAX:{options.MaxFileSize.Value}");
        }
        
        if (options.MinFileSize.HasValue)
        {
            args.Add($"/MIN:{options.MinFileSize.Value}");
        }
        
        // Logging options
        if (!string.IsNullOrEmpty(options.LogFilePath))
        {
            args.Add($"/LOG:{QuotePath(options.LogFilePath)}");
        }
